  ...
  (with-open [model (ModelFactory/createDefaultModel)]
    (testing "Conversion of literals"
      (testing "Conversion of plain literals"
        (are [literal clj] (= (get (store/resource->clj literal) "@value") clj)
             (.createLiteral model "word") "word"
             (.createLiteral model "word" "en") "word"))
      (testing "Conversion of typed literals"
        (are [literal datatype clj] (= (get (store/resource->clj (.createTypedLiteral model
                                                                                      literal
                                                                                      datatype))
                                            "@value") clj)
      ...
      (is (= (get (store/resource->clj (.createResource model "http://www.w3.org/2001/XMLSchema#")) "@id")
